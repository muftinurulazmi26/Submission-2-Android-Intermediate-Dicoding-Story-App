    ...
    private fun init() {
        ...
        addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) {
                if (s.toString().isNotEmpty() && inputType != 129) showClearButton() else hideClearButton()
                if(inputType == 129 && s.toString().length < 6) error = "Minimum password is less than 6"
                if (inputType == 33 && !Patterns.EMAIL_ADDRESS.matcher(s.trim().toString()).matches()) error = "Not valid email"
            }
